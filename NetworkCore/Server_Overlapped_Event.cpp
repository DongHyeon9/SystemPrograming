...
bool Server_Overlapped_Event::Initialize()
{
	std::cout << "Initialize Server Overlapped Event" << std::endl;
...
void Server_Overlapped_Event::Deinitialize()
{
    ::closesocket(socket);
	__super::Deinitialize();
}

void Server_Overlapped_Event::Run()
{
    ...
    while (true)
    {
        ...
        DWORD recvLen{};
        DWORD flags{};
        if (::WSARecv(session.socket, &wsaBuf, BUFFER_COUNT, &recvLen, &flags, &session.overlapped, nullptr) == SOCKET_ERROR)
        {
            if (::WSAGetLastError() == WSA_IO_PENDING)
            {
                // Pending
                ::WSAWaitForMultipleEvents(BUFFER_COUNT, &session.overlapped.hEvent, TRUE, WSA_INFINITE, FALSE);
