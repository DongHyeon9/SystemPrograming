...
int32 __stdcall Listen(SOCKET Socket)
{
...
{
    return ::send(Socket, SendData.c_str(), static_cast<int32>(SendData.size()), 0);
}

int32 __stdcall Recv(SOCKET Socket, std::string& RecvBuffer, size_t BufferSize)
{
    return ::recv(Socket, &RecvBuffer[0], static_cast<int32>(BufferSize), 0);
}
